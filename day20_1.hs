...
pushNTimes :: Int -> Map.Map String Module -> [Signal] -> [Signal]
pushNTimes 0 m sent = sent
pushNTimes n m sent = pushNTimes (n-1) newM (sent ++ newSignals)
  where
    s = Signal "button" "broadcaster" Low
    (newM, newSignals) = handleSignal m [] [s]
...
main = do
  ...
  let signals = pushNTimes 1000 moduleMap' []
  let high = length $ filter (\(Signal _ _ p) -> p == High) signals
  let low = length $ filter (\(Signal _ _ p) -> p == Low) signals
  putStrLn $ "High: " ++ show high ++ ", Low: " ++ show low ++ ", Product: " ++ show (high * low)
