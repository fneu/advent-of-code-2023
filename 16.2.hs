...
startingBeams :: Int -> Int -> [Beam]
startingBeams row columns = 
  map (\column -> Beam (Position 0 column) South) [0..columns-1]
  ++ map (\row -> Beam (Position row 0) East) [0..row-1]
  ++ map (\column -> Beam (Position (row-1) column) North) [0..columns-1]
  ++ map (\row -> Beam (Position row (columns-1)) West) [0..row-1]

poweredTiles :: [String] -> Beam -> Int
poweredTiles input beam = length $ nub $ map position $ lbeamed input [] beam

main :: IO ()
main = do
  input <- fmap lines getContents
  print $ maximum $ map (poweredTiles input) (startingBeams (length input) (length $ head input))
